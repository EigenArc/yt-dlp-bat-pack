...
echo.
echo [確認] 最大解像度: !MAX_HEIGHT!p
echo.
echo --------------------------------------------------
echo ▼ 画質を選択してください
echo --------------------------------------------------
echo [1] 最高画質 (ソースを維持 / 最もおすすめ)
...
echo.
echo ==================================================
:: フォーマット指定の構築 (内側のダブルクォートを排除)
if defined HEIGHT_LIMIT (
    echo [*] ダウンロード開始: 最大 !HEIGHT_LIMIT!p (MKV)
    set "DL_FORMAT=bestvideo[height<=!HEIGHT_LIMIT!]+bestaudio/best"
) else (
