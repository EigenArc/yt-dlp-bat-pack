...
yt-dlp --no-playlist -f "bestvideo[ext=mp4][vcodec^=avc1]" --print "%%(height)s" "!VIDEO_URL!" > "!TMP_INFO!" 2>nul
set "MAX_HEIGHT="
if exist "!TMP_INFO!" (
...
if defined HEIGHT_LIMIT (
    ...
    set "DL_FORMAT=bestvideo[height<=!HEIGHT_LIMIT!][ext=mp4][vcodec^=avc1]+bestaudio[ext=m4a]/bestvideo[height<=!HEIGHT_LIMIT!][ext=mp4][vcodec^=avc1]+bestaudio[ext=mp4]/bestvideo[height<=!HEIGHT_LIMIT!][ext=mp4]+bestaudio[ext=m4a]/bestvideo[height<=!HEIGHT_LIMIT!][ext=mp4]+bestaudio[ext=mp4]/b[height<=!HEIGHT_LIMIT!][ext=mp4]/best"
) else (
    echo [*] ダウンロード開始: 最高画質
